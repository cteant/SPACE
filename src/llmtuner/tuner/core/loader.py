...
# class CustomizeModel(AutoModelForCausalLM):
#     def set_mask_token_and_num(self, mask_id, mask_num):
#         self.mask_id = mask_id
#         self.mask_num = mask_num
#
#     def forward(self, **kwargs):
#         labels = kwargs.pop("labels")
#         model_out = super(AutoModelForCausalLM, self).forward(**kwargs)
#
...
#
#             loss_weight = torch.ones_like(shift_labels)
#             loss_weight[kwargs['input_ids'][:, :-1].contiguous().view(-1) == self.mask_id] = 10
#             # loss_weight[input_ids[:, :-1].contiguous().view(-1) == self.mask_id] = (-torch.cumsum(
#             #     loss_weight[input_ids[:, :-1].contiguous().view(-1) == self.mask_id], -1) % (self.mask_num + 1) + 5) * 2  # [20,18,16,14,12,10]
#
#             loss_none = loss_fct(shift_logits, shift_labels)
#             loss = (loss_none * loss_weight)[shift_labels != -100].mean()
#             loss_mask = loss_none[kwargs['input_ids'][:, :-1].contiguous().view(-1) == self.mask_id].mean()
#
#             logger.info(f"total loss is {loss.item():.3f} mask loss is {loss_mask.item():.3f}")
#             print(f"total loss is {loss.item():.3f} mask loss is {loss_mask.item():.3f}")
#
...
def load_model_and_tokenizer(
    ...
    model_name = model_to_load.split('/')[-1].lower()
    if 'llama' in model_name:  # support efficient training for llama
        ...
        from models.modeling_qwen import QWenLMHeadModel
        ...
        from models.modeling_internlm import InternLMForCausalLM
    ...
    model.set_mask_token_and_num(model_args.mask_id, model_args.mask_num, model_args.mask_loss_weight)
